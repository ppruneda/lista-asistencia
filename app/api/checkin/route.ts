...
export async function POST(request: NextRequest) {
    try {
      ...
      if (!token || !cuenta || !sessionId || !phase) {
      ...
      if (!/^\d{8,10}$/.test(cuenta)) {
              return NextResponse.json(
                {
                            success: false,
                            message: "Número de cuenta inválido (debe tener 8-10 dígitos)",
                },
                { status: 400 }
                      );
      }
      ...
      const studentDoc = await adminDb.collection("students").doc(cuenta).get();
          if (!studentDoc.exists) {
                  return NextResponse.json(
                    {
                                success: false,
                                message: "Tu cuenta no está registrada. Contacta al profesor.",
                    },
                    { status: 400 }
                          );
          }
      ...
      // Device fingerprint checks removed to allow students to register from any device
